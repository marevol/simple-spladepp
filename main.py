...
from simple_spladepp.evaluate import evaluate_with_ranking_loss
from simple_spladepp.model import SimpleSPLADEPlusPlus
from simple_spladepp.train import train_with_ranking_loss
from simple_spladepp.vectorization import SPLADESparseVectorizer
...
def load_model(save_directory="splade_model"):
    model = SimpleSPLADEPlusPlus(model_name=save_directory)
...
# In main.py or wherever you initiate training
def train(logger, train_df, test_df=None, num_train=4000, num_test=800, use_teacher=True):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    logger.info("Initializing tokenizer and model...")
    tokenizer = AutoTokenizer.from_pretrained("Luyu/co-condenser-marco")
    model = SimpleSPLADEPlusPlus(model_name="Luyu/co-condenser-marco").to(device)

    if use_teacher:
        # Initialize the teacher model
        logger.info("Initializing the teacher model...")
        teacher_model = SimpleSPLADEPlusPlus(model_name="bert-large-uncased").to(device) # replace with your teacher model
        teacher_model.eval()  # Set teacher model to evaluation mode
        for param in teacher_model.parameters():
            param.requires_grad = False  # Disable gradient computation for the teacher
    else:
        teacher_model = None
    ...
    logger.info("Starting training with ranking loss and distillation...")
    train_with_ranking_loss(model, dataloader, optimizer, num_epochs=2, device=device, teacher_model=teacher_model)
...
if __name__ == "__main__":
    ...
    logger.info("Starting SPLADE++ training...")
